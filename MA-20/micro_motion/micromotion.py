...
def generate_image_files(directory):
    ...
    image_files = sorted(
        [os.path.join(directory, f) for f in os.listdir(directory) if f.lower().endswith(valid_extensions)])
...
def compare_images(image1_path, image2_path, gray_mode, threshold, fact_reduc=2):
    ...
    start = time.perf_counter()
    if fact_reduc > 1:
        img1 = cv2.resize(img1, (int(img1.shape[1] / fact_reduc), int(img1.shape[0] / fact_reduc)),
                          interpolation=cv2.INTER_NEAREST)
        img2 = cv2.resize(img2, (int(img2.shape[1] / fact_reduc), int(img2.shape[0] / fact_reduc)),
                          interpolation=cv2.INTER_NEAREST)
...
class ImageViewer(tk.Tk):
    def __init__(self, directory):
        ...
        def create_widgets():
            ...
            self.slider = tk.Scale(self.canvas_frame_cmd, from_=0, to=len(self.image_files) - 2, orient=tk.HORIZONTAL,
                                   length=380)
            ...
            self.lst_fact_reduc = tk.OptionMenu(self.canvas_frame_cmd, self.fact_reduc_var, 1, 2, 4, 8, 16,
                                                command=self.update_fact_reduc)
            ...
            self.lst_blob_size = tk.OptionMenu(self.canvas_frame_cmd, self.blob_size_var, 6, 8, 12, 16, 24, 32, 48, 64,
                                               128, command=self.update_blob_size)
            ...
            self.lst_blob_min_percentage = tk.OptionMenu(self.canvas_frame_cmd, self.blob_min_percentage_var, 10, 15,
                                                         20, 30, 40, 50, 75, 100,
                                                         command=self.update_blob_min_percentage)
            self.lst_blob_min_percentage.grid(row=6, column=1, padx=10, pady=10, sticky='w')
            self.label_blob_min_percentage = tk.Label(self.canvas_frame_cmd, text="% différence min / tache",
                                                      font=("Arial", 8))
            ...
            # Zone d'analyse
            self.cmd_edit_polygon = tk.Button(self.canvas_frame_cmd, text="Définir zone d'analyse (clic/clic droit)",
                                              command=self.edit_polygon)
    ...
    def edit_polygon(self):
        ...
        if self.editing_polygon:  # si on passe en édition
            ...
            print("Zone d'analyse mémorisée :", self.points, "editing_polygon",
                  self.editing_polygon)  # Affichage des coordonnées
    ...
    def update_image(self):
        ...
        print()
        print("Traitement de", img1, img2, "facteur de différence", self.threshold, "%", "taille des taches",
              self.blob_size, "pixels", "pourcentage de taches", self.blob_min_percentage * 100, "%")

        # Comparer les images et créer l'image de différence
        base_image, self.diff_image, self.img_sizex = compare_images(img1, img2, self.gray_mode, self.threshold,
                                                                     self.fact_reduc)
        ...
        self.label.config(
            text=f"Images analysées : {self.image_files[self.current_index]} - {self.image_files[self.current_index + 1]}")
    ...
    def display_blobs(self):
        if (self.zoom_factor >= 1):
            ...
            for mvt_blob in self.mvt_blobs:
                self.canvas.create_oval((mvt_blob[1] - self.blob_size) / factor,
                                        (mvt_blob[0] - self.blob_size) / factor,
                                        (mvt_blob[1] + mvt_blob[3] + self.blob_size) / factor,
                                        (mvt_blob[0] + mvt_blob[2] + self.blob_size) / factor, outline='blue', width=2)
    ...
    def zoom_in(self, event):
        ...
        if self.zoom_factor <= 8:
            ...
            self.canvas.xview_moveto(
                (mouse_x / self.old_zoom) / canvas_width * (self.zoom_factor - 1) / self.zoom_factor)
            self.canvas.yview_moveto(
                (mouse_y / self.old_zoom) / canvas_height * (self.zoom_factor - 1) / self.zoom_factor)
