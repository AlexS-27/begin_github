...
def find_fast_solution(text_widget):
    """Optimized function to solve SPORT + EFFORT = PLAISIR efficiently"""
    try:
        ...
        p, e, l = 1, 9, 0  # Given constraints

        # Generate all unique permutations of the remaining digits
        for perm in itertools.permutations([2, 3, 4, 5, 6, 7, 8], 7):
            a, f, i, o, s, r, t = perm  # Unpack values

            # Form full numbers using precomputed values
            sport = s * 10000 + p * 1000 + o * 100 + r * 10 + t
            effort = e * 100000 + f * 10000 + f * 1000 + o * 100 + r * 10 + t
            plaisir = p * 1000000 + l * 100000 + a * 10000 + i * 1010 + s * 100 + r
...
btn_calculate = tk.Button(root, text="sport smart fast", command=find_sep_smart)
btn_calculate.grid(row=3, column=1, pady=10)
