...
class ImageViewer(tk.Tk):
    ...
    def update_image(self):
        ...
        print()
        print("Traitement de", image1_path, image2_path, "facteur de différence", self.threshold, "%",
              "taille des taches", self.blob_size, "pixels", "pourcentage de taches", self.blob_min_percentage * 100,
              "%")
        ...
        if self.fact_reduc > 1:
            self.img1 = cv2.resize(self.img1, (
            int(self.img1.shape[1] / self.fact_reduc), int(self.img1.shape[0] / self.fact_reduc)),
                                   interpolation=cv2.INTER_NEAREST)
            self.img2 = cv2.resize(self.img2, (
            int(self.img2.shape[1] / self.fact_reduc), int(self.img2.shape[0] / self.fact_reduc)),
                                   interpolation=cv2.INTER_NEAREST)
            print_time("Réduction facteur " + str(self.fact_reduc), start)

        # Comparer les images et créer l'image de différence
        base_image, self.diff_image, self.img_sizex = compare_images(self.img1, self.img2, self.gray_mode,
                                                                     self.threshold)
        ...
        print_time("Détection taches :" + str(self.blob_size) + "/" + str(self.blob_min_percentage) + "=>" + str(
            len(self.mvt_blobs)), start)
            ...
            if (self.zoom_factor >= 1):
                ...
                for mvt_blob in self.mvt_blobs:
                    self.canvas.create_oval((mvt_blob[1] - self.blob_size) / factor,
                                            ...
                                            (mvt_blob[0] + mvt_blob[2] + self.blob_size) / factor, outline='blue',
                                            width=2)
