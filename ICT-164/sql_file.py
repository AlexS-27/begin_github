...
# fonction qui renvoie true si la donnée est un nombre entier ou décimal
...
def generate_sql_from_text():
    ...
    table_name = entry_table.get().strip()  # Vous pouvez personnaliser ou demander une saisie
    #if not table_name:
    ...
    lines = content.split("\n")
    if len(lines) < 2:
        messagebox.showwarning("Attention", f"Le contenu doit contenir une ligne d'en tête et des lignes de données")
        return
    ...
    data_rows = [lines.split(";") for lines in lines[1:] if lines.strip()]
    print(data_rows)
